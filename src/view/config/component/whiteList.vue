  ...
  <div>
    ...
    <Row class="table-container">
      ...
      >
        ...
        <template slot-scope="{ row }" slot="hostName">
          <span style="font-size: 12px;color: #666">
            <span style="color: #00e9bc;">{{ row.hostName || 'unknow' }}</span>
          </span>
        </template>
        ...
        <template slot-scope="{ row, index }" slot="action">
          <Icon
            ...
            @click="changeIpStatus(row, index)"
          />
          <Icon
            type="ios-trash"
            size="24"
    ...
    <Row class="table-container">
      ...
      >
        ...
        <template slot-scope="{ row }" slot="ipAddress">
          <span style="font-size: 12px;color: #666">
            <span style="color: #00e9bc;">{{ row.ipAddress || 'unknow' }}</span>
          </span>
        </template>
        <template slot-scope="{ row }" slot="hostName">
          <span style="font-size: 12px;color: #666">
    ...
    <Modal v-model="addWhiteModel" width="360">
      ...
      <div style="text-align:center">
        <Form
          :model="addWhiteForm"
          label-position="left"
          ref="whiteFormValidate"
          :rules="whiteFormRules"
        >
          ...
          <FormItem label="导入表格">
            ...
            <a
              :href="$config.baseUrl.pro+'/'+download.name"
              target="_blank"
              :download="download.name"
            >
              <Icon type="ios-cloud-download-outline" />下载模板
            </a>
            <Row>
      ...
      <div slot="footer">
        <Button
          ...
          size="large"
          long
          :loading="addWhiteLoading"
          @click="handleSubmit('whiteFormValidate')"
        >确认添加</Button>
      </div>
    </Modal>
    <!--修改别名-->
...
import { uptRosterTemp, uptRosterAll } from "../../../api/ipManage";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      whiteFormRules: {
        macAdress: [{ validator: macAddressRules, trigger: "blur" }],
        ipAdress: [{ validator: ipAddress, trigger: "blur" }]
      },
      editName: false,
      editNameForm: {},
      reBackIpModel: false,
  ...
  methods: {
    ...
    async getAllNameListAuto() {
      this.loading = true;
      let res = await getAllNameListAuto({
        nbCode: this.nbCode,
        type: 4,
        groupId: this.groupId
      });
    ...
    async addIp() {
      ...
      type = 4;
      if (
        this.addWhiteForm.ipAdress === "" &&
        this.addWhiteForm.macAdress === ""
      ) {
        this.upload(type);
        return;
      }
    ...
    async selRGInfoByNbCode() {
      ...
      if (res.data.code === "success") {
        //  console.log(res.data.result);
        this.groupNameList = res.data.result || [];
      }
    },
    //获取选中的值查询数据
    getChangeValue(value) {
      ...
      if (value !== "null") {
        for (let i = 0; i < this.groupNameList.length; i++) {
          if (this.groupNameList[i].groupName == value) {
            this.groupId = this.groupNameList[i].id;
          }
        }
      } else {
        this.groupId = "";
